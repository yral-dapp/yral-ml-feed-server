...
use yral_ml_feed_cache::{
    consts::{
        GLOBAL_CACHE_CLEAN_KEY, MAX_GLOBAL_CACHE_LEN, MAX_WATCH_HISTORY_CACHE_LEN,
        USER_CACHE_CLEAN_SUFFIX, USER_WATCH_HISTORY_CLEAN_SUFFIX,
    },
    types::PostItem,
    MLFeedCacheState,
};
...
pub async fn get_coldstart_clean_cache_noinput_impl(
    ...
    let num_posts_in_cache = ml_feed_cache
        .get_cache_items_len(GLOBAL_CACHE_CLEAN_KEY)
        .await
    ...
    let feed = ml_feed_cache
        .get_cache_items(GLOBAL_CACHE_CLEAN_KEY, post_index, post_index + 1)
        .await
...
pub async fn get_coldstart_clean_cache_noinput_user_impl(
    ...
    let num_posts_in_cache = ml_feed_cache
        .get_cache_items_len(&format!("{}{}", canister_id, USER_CACHE_CLEAN_SUFFIX))
        .await
        .map_err(|e| anyhow::anyhow!("Failed to get num posts in cache: {}", e))?;
    ...
    let feed = ml_feed_cache
        .get_cache_items(
            &format!("{}{}", canister_id, USER_CACHE_CLEAN_SUFFIX),
            post_index,
            post_index + 1,
        )
        .await
...
pub async fn get_coldstart_clean_cache_input_impl(
    ...
    let global_cache_clean_feed = ml_feed_cache
        .get_cache_items(GLOBAL_CACHE_CLEAN_KEY, 0, MAX_GLOBAL_CACHE_LEN)
        .await
        .map_err(|e| anyhow::anyhow!("Failed to get global cache clean feed: {}", e))?;

    Ok(get_shuffled_limit_list(global_cache_clean_feed, num_results as usize).await)
}
